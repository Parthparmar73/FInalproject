...
const MY_ROUTE = 'digitaltransformation';
...
export class Digitaltransformation implements OnInit {
  showPayment = false;
  showQuote = false;
  pkgLoaded = false;

  readonly defaultName     = 'Digital Transformation';
  readonly defaultIcon     = '♻️';
  readonly defaultFeatures = ['Workflow Automation', 'Cloud Migration', 'Data Analytics'];
  readonly defaultPrice    = 5000;
  ...
  get pkgDesc()     { return this.pkg.description || 'Helping businesses evolve by integrating digital technology into all areas.'; }
  get pkgDisplayId() {
  ...
  get iconGradient(): string {
    ...
    return map[this.pkg.badgeColor || ''] || 'linear-gradient(135deg,#3b82f6,#1d4ed8)';
  }
