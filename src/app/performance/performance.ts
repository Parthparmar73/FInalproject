...
export class Performance implements OnInit {
  ...
  readonly defaultName     = 'Performance';
  readonly defaultIcon     = '⚡';
  readonly defaultFeatures = ['Core Web Vitals Boost', 'Lazy Loading', 'CDN Integration'];
  readonly defaultPrice    = 2000;
  ...
  get pkgDesc()     { return this.pkg.description || 'Optimizing web application speed and core web vitals for better user experience.'; }
  ...
  get iconGradient(): string {
    ...
    return map[this.pkg.badgeColor || ''] || 'linear-gradient(135deg,#a855f7,#7c3aed)';
  }
