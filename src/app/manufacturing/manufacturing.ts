...
export class Manufacturing implements OnInit {
  ...
  readonly defaultName     = 'Manufacturing';
  readonly defaultIcon     = '🏭';
  readonly defaultFeatures = ['ERP Integration', 'Inventory Automation', 'Quality Control'];
  readonly defaultPrice    = 8000;
  ...
  constructor(private auth: Auth, private router: Router, private cdr: ChangeDetectorRef) {
    ...
    if (state) { this.pkg = typeof state === 'string' ? JSON.parse(state) : state; this.pkgLoaded = true; }
  }
  ...
  get pkgDesc()     { return this.pkg.description || 'Digital solutions tailored for manufacturing operations, ERP and supply chain.'; }
  ...
  get iconGradient(): string {
    ...
    return map[this.pkg.badgeColor || ''] || 'linear-gradient(135deg,#0891b2,#0e7490)';
  }
